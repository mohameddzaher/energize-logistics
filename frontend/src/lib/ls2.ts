...
export function ls2Text(lang: Lang) {
  ...
  return {
    ...
    servicedAt: t('serviced at', 'نُفِّذت عند'),
    ...
    deferralDone: t('Done now', 'تم التنفيذ'),
    deferralDoneHint: t('It was done. Close it and clear the alert.', 'نُفِّذ فعلًا — يُغلَق البند ويُغلَق تنبيهه.'),
    deferralDefer: t('Defer again', 'تأجيل إضافي'),
    deferralDeferHint: t('Still fine — grant it more km.', 'ما زالت بحالة جيدة — تُمنَح مسافة إضافية.'),
    deferralSkip: t("Won't do", 'لن يُنفَّذ'),
    deferralSkipHint: t('Decided not to do it. Close it.', 'تقرر عدم تنفيذه — يُغلَق البند.'),
    deferralAddKm: t('Extra km', 'مسافة إضافية (كم)'),
    deferralOdoNow: t('Odometer now (km) — optional', 'العداد الحالي (كم) — اختياري'),
