...
export default function PayablesPage() {
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <PageHeader icon={<CreditCard className="w-5 h-5" />} title={ar ? 'الذمم الدائنة' : 'Payables (A/P Aging)'} subtitle={money(data.total)} />
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-x-auto shadow-sm">
        <table className="w-full text-sm">
          <thead><tr className="bg-slate-900 border-b border-slate-200 text-left text-slate-300">
            <th className="px-4 py-3">{ar ? 'الفاتورة' : 'Bill'}</th>
            <th className="px-4 py-3">{ar ? 'المورّد' : 'Vendor'}</th>
            <th className="px-4 py-3 text-right">{ar ? 'المتبقي' : 'Balance'}</th>
