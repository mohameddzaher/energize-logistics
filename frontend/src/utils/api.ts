// import { Booking, BookingFormData, AuthResponse, AdminBooking } from '../types';
...
//   getAll: async (): Promise<{ status: string; data: { bookings: Booking[] } }> => {
...
//   getAvailable: async (date: string): Promise<any> => {
//     const token = localStorage.getItem('token');
//     const response = await fetch(`${API_BASE_URL}/bookings/available?date=${date}`, {
//       headers: {
//         'Authorization': `Bearer ${token}`,
//       },
//     });
...
//     // علشان الـ 204 No Content مافيش JSON
//     if (response.status === 204) {
//       return { status: 'success', data: null };
//     }
    
//     // لو فيه error، حاول parse الـ JSON
//     try {
