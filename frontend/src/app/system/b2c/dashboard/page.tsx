...
export default function B2CDashboard() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
              showFilters ? 'bg-[#f37121] text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'
            }`}>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 shadow-sm">
              ...
              <div className="col-span-1 sm:col-span-2 md:col-span-4 lg:col-span-7 flex justify-end">
                <button type="button" onClick={clearFilters} className="px-3 py-1.5 text-slate-500 hover:text-slate-900 text-sm">
                  {T.clearFilters}
                </button>
              </div>
            </div>
      ...
      {allMonths.length > 0 && (
        <div dir="ltr" className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-2 overflow-x-auto shadow-sm">
          <button type="button"
      ...
      <div className="flex items-center gap-1 border-b border-slate-200 -mx-4 sm:mx-0 px-4 sm:px-0 overflow-x-auto">
        {[
                    ...
                    <button key={r.repId || i} type="button" onClick={() => r.repId && openRepProfile(r.repId)}
                      ...
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                          ...
                          i === 2 ? 'bg-orange-700/30 text-orange-600' :
                          'bg-slate-100 text-slate-500'
                        }`}>{i + 1}</span>
                        <div className="min-w-0">
                          <p className="text-slate-900 text-sm font-medium truncate">{r.englishName}</p>
                ...
                <tbody className="divide-y divide-slate-200">
                  {dashboard.byRep.map((r, i) => (
                    <tr key={r.repId || i} onClick={() => r.repId && openRepProfile(r.repId)}
                      ...
                      <td className="py-2 px-3 text-center text-slate-900 font-semibold">{r.totalOrders.toLocaleString()}</td>
                      <td className="py-2 px-3 text-center text-slate-700">{r.workingDays}</td>
                      <td className="py-2 px-3 text-center text-slate-700">{r.dailyRate.toFixed(1)}</td>
...
function HighlightCard({ title, value, subtitle, icon, color, onClick }: any) {
  ...
  return (
    <Wrapper onClick={onClick} type={onClick ? 'button' : undefined}
      ...
      <div className="flex items-start justify-between">
        <div className="min-w-0 flex-1">
          <p className="text-slate-500 text-xs uppercase font-medium">{title}</p>
          <p className="text-slate-900 text-lg font-bold mt-1 truncate">{value}</p>
...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
      ...
      <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3 text-sm">{title}</h3>
      {children}
    </motion.div>
  );
}
...
function EvaluationsView({ evaluations, T, lang, onRepClick }: any) {
  ...
  const trendIcon = (t: string) => {
    if (t === 'rising') return <TrendingUp className="w-3.5 h-3.5 text-green-600" />;
    if (t === 'falling') return <TrendingDown className="w-3.5 h-3.5 text-red-600" />;
            ...
            <tbody className="divide-y divide-slate-200">
              {filtered.map((e: any, i: number) => (
                <tr key={e.repId} onClick={() => onRepClick(e.repId)} className="hover:bg-slate-100 cursor-pointer transition-colors">
                  ...
                  <td className="py-2 px-3 text-center text-slate-900 font-semibold">{e.totalOrders.toLocaleString()}</td>
                  <td className="py-2 px-3 text-center text-slate-700">{e.monthsActive}</td>
                  <td className="py-2 px-3 text-center text-slate-700">{e.avgPerformance.toFixed(1)}%</td>
                  <td className="py-2 px-3 text-center text-slate-700">{e.consistency.toFixed(0)}%</td>
                  <td className="py-2 px-3 text-center">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {topPerformers.length === 0 ? (
              <p className="text-slate-500 text-sm text-center py-2">{T.noData}</p>
            ...
            {byDayOfWeek.length === 0 ? (
              ...
              return byDayOfWeek.map((d: any) => (
                <div key={d.dow} className="flex items-center gap-2">
                  <span className="text-slate-700 text-xs w-12">{dowLabels[d.dow]}</span>
  ...
  return (
    <div className="space-y-4">
      ...
      {sortedMonths.map(([monthKey, daysOfMonth]) => (
        <div key={monthKey} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          {!isSingleMonth && (
            <p className="text-slate-700 text-xs font-medium uppercase mb-3">{monthLabel(monthKey)}</p>
          )}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-2">
            {daysOfMonth.map((d: any) => {
              ...
              // Stripe color rules
              let stripe = 'bg-slate-100';
              if (!hasData) stripe = 'bg-slate-100';
                ...
                >
                  ...
                  <div className="p-2.5">
                    <div className="flex items-baseline justify-between">
                      <span className="text-slate-900 text-lg font-bold">{day}</span>
                      <span className="text-slate-500 text-[10px]">{dowLabel}</span>
                    </div>
                    {hasData ? (
                      <>
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="bg-white border border-slate-200 rounded-xl w-full max-w-3xl shadow-2xl max-h-[92vh] overflow-y-auto"
          ...
          <div className="p-5">
            {loading ? (
              ...
              <p className="text-slate-500 text-sm text-center py-6">{T.noData}</p>
            ) : (
              <div className="space-y-4">
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {data.reps.length === 0 ? (
                        ...
                        <tr key={i} onClick={() => r.repId && onRepClick(r.repId)} className="cursor-pointer hover:bg-slate-100">
                          ...
                          <td className="py-2 px-3 text-slate-700 text-sm">{r.project || '—'}</td>
                          <td className="py-2 px-3 text-center text-slate-900 font-semibold">
...
function DayKpi({ label, value, color }: { label: string; value: any; color: string }) {
  return (
    <div className="bg-slate-100 rounded-lg p-3">
      <p className="text-slate-500 text-[11px] uppercase">{label}</p>
