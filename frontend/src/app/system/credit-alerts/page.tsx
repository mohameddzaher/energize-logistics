  ...
  const columns = [
    {
      ...
      render: (_: any, row: any) => (
        <div>
          <span className="text-white font-medium text-sm">{row.companyName}</span>
          {row.customerNumber && <span className="text-gray-500 text-xs block">{row.customerNumber}</span>}
        </div>
      ),
    },
    {
      key: 'creditLimit',
      label: T.creditLimit,
      render: (val: number) => <span className="text-white text-sm">{'SAR ' + Math.round(val || 0).toLocaleString('en-US')}</span>,
    },
    {
      key: 'currentOutstanding',
      label: T.currentOutstanding,
      render: (val: number) => <span className="text-red-400 text-sm font-medium">{'SAR ' + Math.round(val || 0).toLocaleString('en-US')}</span>,
    },
    {
      key: 'remaining',
      label: 'Remaining',
      render: (val: number, row: any) => (
        <span className={`text-sm font-medium ${val < 0 ? 'text-red-400' : val < row.creditLimit * 0.2 ? 'text-yellow-400' : 'text-green-400'}`}>
    ...
    },
    {
      ...
      render: (val: number) => (
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-bold ${val > 100 ? 'text-red-400' : val >= 90 ? 'text-orange-400' : 'text-yellow-400'}`}>
            {val}%
          </span>
        </div>
      ),
    },
    {
      ...
      label: 'Grade',
      render: (val: string) => {
        const colors: Record<string, string> = { A: 'text-green-400 bg-green-500/20', B: 'text-blue-400 bg-blue-500/20', C: 'text-yellow-400 bg-yellow-500/20', D: 'text-red-400 bg-red-500/20' };
        return <span className={`px-2 py-0.5 rounded text-xs font-medium ${colors[val] || 'text-gray-400 bg-gray-500/20'}`}>{val || '-'}</span>;
      },
    },
    {
      key: 'creditTerm',
      label: 'Credit Term',
      render: (val: number) => <span className="text-gray-300 text-sm">{val} days</span>,
    },
    {
    ...
    },
    {
      ...
      render: (val: string, row: any) => val ? (
        <div>
          <span className="text-gray-300 text-xs">{new Date(val).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
          {row.lastPaymentAmount && <span className="text-green-400 text-xs block">{'SAR ' + Math.round(row.lastPaymentAmount).toLocaleString('en-US')}</span>}
        </div>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
          <p className="text-red-400 text-xs uppercase font-medium">{T.overLimit}</p>
          <p className="text-2xl font-bold text-red-400 mt-1">{data?.exceeded || 0}</p>
          <p className="text-red-400/60 text-xs mt-1">Over 100% usage</p>
        </div>
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4">
          <p className="text-yellow-400 text-xs uppercase font-medium">Near Limit</p>
          <p className="text-2xl font-bold text-yellow-400 mt-1">{data?.nearLimit || 0}</p>
          <p className="text-yellow-400/60 text-xs mt-1">80-100% usage</p>
        </div>
