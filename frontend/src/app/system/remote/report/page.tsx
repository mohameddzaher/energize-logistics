...
function EmployeeReport({ ar, isRTL }: { ar: boolean; isRTL: boolean }) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <h1 className="text-2xl font-bold text-white flex items-center gap-2"><FileText className="w-6 h-6 text-[#f37121]" />{ar ? 'التقرير اليومي' : 'Daily Report'}</h1>
      <form onSubmit={submit} className="bg-gray-800 border border-gray-700 rounded-xl p-5 space-y-4">
        <p className="text-gray-400 text-sm">{ar ? `تقرير اليوم (${today})` : `Today's report (${today})`}</p>
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={6} placeholder={ar ? 'اكتب ماذا عملت اليوم...' : 'Write what you did today...'} className="w-full px-3 py-3 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#f37121]/50" />
        <div className="flex items-center justify-between">
          {savedMsg ? <span className="text-green-400 text-sm">{savedMsg}</span> : <span />}
      ...
      <div>
        <h2 className="text-white font-semibold mb-2">{ar ? 'تقاريري السابقة' : 'My Previous Reports'}</h2>
        {loading ? (
          ...
          <p className="text-gray-400 text-sm py-4">{ar ? 'لا توجد تقارير بعد' : 'No reports yet'}</p>
        ) : (
          <div className="space-y-2">
            {reports.map((r) => (
              <div key={r._id} className="bg-gray-800 border border-gray-700 rounded-xl p-4">
                <p className="text-gray-400 text-xs mb-1">{r.date}</p>
...
function StaffReports({ ar, isRTL }: { ar: boolean; isRTL: boolean }) {
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2"><FileText className="w-6 h-6 text-[#f37121]" />{ar ? 'تقارير الفريق اليومية' : 'Team Daily Reports'}</h1>
      ...
      {loading ? (
        ...
        <p className="text-gray-400 text-center py-10">{ar ? 'لا توجد تقارير' : 'No reports'}</p>
      ) : (
        <div className="space-y-2">
          {reports.map((r) => (
            <div key={r._id} className="bg-gray-800 border border-gray-700 rounded-xl p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-white font-medium text-sm">{r.user ? `${r.user.firstName} ${r.user.lastName}` : '—'}</span>
