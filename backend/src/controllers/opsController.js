...
exports.shipmentTimeline = async (req, res) => {
  try {
    ...
    // The "requesting" step IS the shipment's creation. UPL is inconsistent here:
    // it often leaves this step's `admin` null, and SOMETIMES its `created_at` null
    // too — even though the shipment plainly has both `created_by` and `created_at`.
    // Backfill both from the shipment so "requesting" always shows when + who,
    // like the later steps. The caller passes the shipment fields it already has,
    // so we only hit UPL again as a fallback.
    const requesting = items && items.find((e) => e && e.status === 'requesting');
    const needsWho = requesting && !(requesting.admin && requesting.admin.name);
    const needsWhen = requesting && !requesting.created_at;
    if (requesting && (needsWho || needsWhen)) {
      try {
        let createdBy = req.query.created_by;
        let creatorType = req.query.creator_type;
        let createdAt = req.query.created_at;
        if ((needsWho && !createdBy) || (needsWhen && !createdAt)) {
          const shipRes = await upl.get(`/admin/shipments/${encodeURIComponent(id)}`, {});
          const ship = shipRes.data ?? shipRes;
          createdBy = createdBy || (ship && ship.created_by);
          creatorType = creatorType || (ship && ship.creator_type);
          createdAt = createdAt || (ship && ship.created_at);
        }
        if (needsWhen && createdAt) requesting.created_at = createdAt;
        if (needsWho) {
