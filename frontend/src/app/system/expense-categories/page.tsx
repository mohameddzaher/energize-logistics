    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          ], `expense-categories-${new Date().toISOString().split('T')[0]}`, 'Expense Categories')} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm transition-colors">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-900 border-b border-slate-200">
              ...
              <th className="text-left text-slate-300 font-semibold px-4 py-3">{T.description}</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={isSuperAdmin ? 4 : 3} className="text-center text-slate-500 py-12">{T.noCategories}</td></tr>
            ) : filtered.map((c) => (
