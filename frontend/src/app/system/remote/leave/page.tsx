  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><CalendarCheck className="w-6 h-6 text-[#f37121]" />{ar ? 'الإجازات' : 'Leave'}</h1>
      ...
      {staff && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 shadow-sm">
          ...
          <div>
            <label className="block text-slate-500 text-xs mb-1">{ar ? 'الحالة' : 'Status'}</label>
            <select aria-label="Status" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm">
              <option value="">{ar ? 'الكل' : 'All'}</option>
      ...
      {loading ? (
        ...
        <p className="text-slate-500 text-center py-10">{ar ? 'لا توجد طلبات إجازة' : 'No leave requests'}</p>
      ) : (
            ...
            return (
              <div key={l._id} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-sm">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 text-sm">{l.startDate} → {l.endDate} · {l.days} {ar ? 'يوم' : 'day(s)'}</p>
                  {l.reason && <p className="text-slate-500 text-sm">{l.reason}</p>}
                  {l.status !== 'pending' && l.reviewedBy && (
                    <p className="text-slate-500 text-xs">{ar ? 'بواسطة' : 'by'} {l.reviewedBy.firstName} {l.reviewedBy.lastName}</p>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-white border border-slate-200 rounded-xl w-full max-w-md shadow-2xl" dir={isRTL ? 'rtl' : 'ltr'}>
            <div className="flex items-center justify-between p-5 border-b border-slate-200">
              <h2 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold text-lg mb-3">{ar ? 'طلب إجازة' : 'Request Leave'}</h2>
            ...
            <form onSubmit={submit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-slate-700 text-sm mb-1.5">{ar ? 'نوع الإجازة' : 'Leave Type'}</label>
                <select aria-label="Type" value={type} onChange={(e) => setType(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm">
                  {LEAVE_TYPES.map((t) => <option key={t.key} value={t.key}>{ar ? t.ar : t.en}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 text-sm mb-1.5">{ar ? 'من' : 'From'}</label>
                  <input aria-label="From" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm" required />
                </div>
                <div>
                  <label className="block text-slate-700 text-sm mb-1.5">{ar ? 'إلى' : 'To'}</label>
                  <input aria-label="To" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm" required />
                </div>
              </div>
              <div>
                <label className="block text-slate-700 text-sm mb-1.5">{ar ? 'السبب (اختياري)' : 'Reason (optional)'}</label>
                <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm" placeholder={ar ? 'اكتب السبب...' : 'Write a reason...'} />
              </div>
