...
function buildWorkflowFilter(query, skipField, allowMoney = true) {
  ...
  // ── «كشفٌ فُوتِر ولا رقمَ لفاتورته» ─────────────────────────────────────
  //
  // خانةُ رقم الفاتورة تُكتب بيدٍ عندنا ولا تأتي من منصّة التشغيل، وهي فارغةٌ
  // في ثلاثين ألفَ كشفٍ من خمسةٍ وثلاثين. وأكثرُها لم يُفوتَر بعدُ حقًّا، فليست
  // كلُّها نقصًا.
  //
  // لكنّ طائفةً منها فُوتِرت فعلًا — عليها صافٍ أو ضريبةٌ أو إجماليّ — ونُسي
  // رقمُها. تلك نقصٌ صريحٌ يُسَدّ: بالرقم تُعرف كشوفُ الفاتورة في قسم التحصيل،
  // وبدونه تبقى الفاتورةُ تقول «لا كشوف» وهي تحمل كشوفًا.
  if (invoiceGap === 'true') {
    filter.$and = [...(filter.$and || []), {
      ...
      $or: [
        { netInvoice: { $gt: 0 } }, { tax: { $gt: 0 } }, { totalInvoice: { $gt: 0 } },
      ],
    }];
  }
