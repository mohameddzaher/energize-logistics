...
export default function PurchaseRequestsPage() {
  ...
  const canApprove = isProcManager(user?.role);
  ...
  const decide = async (pr: PurchaseRequest, decision: string) => {
    if (decision === 'rejected' && !confirm(ar ? 'رفض الطلب؟' : 'Reject request?')) return;
    try { await api.patch(`/api/procurement/requests/${pr._id}/decision`, { decision }); load(); } catch (e: any) { alert(e.message); }
  };
  const remove = async (pr: PurchaseRequest) => { if (!confirm(ar ? 'حذف الطلب؟' : 'Delete request?')) return; try { await api.delete(`/api/procurement/requests/${pr._id}`); load(); } catch (e: any) { alert(e.message); } };
  const convert = async () => {
    if (!convVendor) { alert(ar ? 'اختر مورّد' : 'Pick a vendor'); return; }
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <PageHeader icon={<ClipboardList className="w-5 h-5" />} title={ar ? 'طلبات الشراء' : 'Purchase Requests'} subtitle={`${items.length}`}>
        <PrimaryButton onClick={openCreate}><Plus className="w-4 h-4" /> {ar ? 'طلب جديد' : 'New Request'}</PrimaryButton>
      </PageHeader>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-x-auto shadow-sm">
        <table className="w-full text-sm">
          <thead><tr className="bg-slate-900 border-b border-slate-200 text-left text-slate-300">
            <th className="px-4 py-3">#</th><th className="px-4 py-3">{ar ? 'العنوان' : 'Title'}</th>
            <th className="px-4 py-3">{ar ? 'مقدّم الطلب' : 'Requester'}</th><th className="px-4 py-3 text-right">{ar ? 'التقدير' : 'Estimate'}</th>
            <th className="px-4 py-3">{ar ? 'الأولوية' : 'Priority'}</th><th className="px-4 py-3">{ar ? 'الحالة' : 'Status'}</th>
            ...
            {items.length === 0 ? <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">—</td></tr> : items.map((pr) => (
              <tr key={pr._id} className="hover:bg-slate-100">
                ...
                <td className="px-4 py-3"><div className="flex items-center justify-end gap-2">
                  {pr.status === 'draft' && <button type="button" title={ar ? 'إرسال للموافقة' : 'Submit'} onClick={() => submitPR(pr)} className="text-amber-700 hover:text-amber-700"><Send className="w-4 h-4" /></button>}
                  {pr.status === 'pending_approval' && canApprove && <>
                    <button type="button" title={ar ? 'اعتماد' : 'Approve'} onClick={() => decide(pr, 'approved')} className="text-green-600 hover:text-green-700"><Check className="w-4 h-4" /></button>
                    <button type="button" title={ar ? 'رفض' : 'Reject'} onClick={() => decide(pr, 'rejected')} className="text-red-600 hover:text-red-700"><X className="w-4 h-4" /></button>
                  </>}
                  {pr.status === 'approved' && <button type="button" title={ar ? 'تحويل لأمر شراء' : 'Convert to PO'} onClick={() => { setConvertFor(pr); setConvVendor(''); }} className="text-blue-600 hover:text-blue-700"><ShoppingCart className="w-4 h-4" /></button>}
                  {['draft', 'pending_approval'].includes(pr.status) && <button type="button" title={ar ? 'تعديل' : 'Edit'} onClick={() => openEdit(pr)} className="text-blue-600 hover:text-blue-700"><Edit className="w-4 h-4" /></button>}
                  {canApprove && <button type="button" title={ar ? 'حذف' : 'Delete'} onClick={() => remove(pr)} className="text-red-600 hover:text-red-700"><Trash2 className="w-4 h-4" /></button>}
                </div></td>
              </tr>
            ))}
          </tbody>
      ...
      {/* Create / edit */}
      <Modal open={showModal} onClose={() => setShowModal(false)} wide title={editing ? (ar ? 'تعديل طلب' : 'Edit Request') : (ar ? 'طلب شراء جديد' : 'New Purchase Request')}
        footer={<><button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm">{ar ? 'إلغاء' : 'Cancel'}</button>
          <button type="button" onClick={() => save(false)} disabled={saving} className="px-4 py-2 bg-slate-200 text-slate-900 rounded-lg text-sm disabled:opacity-50">{ar ? 'حفظ كمسودة' : 'Save Draft'}</button>
          <PrimaryButton onClick={() => save(true)} disabled={saving}>{saving ? '...' : (ar ? 'إرسال للموافقة' : 'Submit')}</PrimaryButton></>}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label={ar ? 'العنوان' : 'Title'} span2><TextInput value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></Field>
          ...
          <Field label={ar ? 'القسم' : 'Department'}><TextInput value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })} /></Field>
          <Field label={ar ? 'الأولوية' : 'Priority'}><Select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })}>
            {(opts?.PRIORITIES || []).map((c) => <option key={c.key} value={c.key}>{ar ? c.nameAr : c.nameEn}</option>)}
          </Select></Field>
          <Field label={ar ? 'مطلوب بحلول' : 'Needed By'}><TextInput type="date" value={form.neededBy} onChange={(e) => setForm({ ...form, neededBy: e.target.value })} /></Field>
        </div>
        {/* line items */}
        <div className="mt-2 space-y-2">
          ...
          {form.items.map((l: any, i: number) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <input type="number" placeholder={ar ? 'كمية' : 'Qty'} value={l.quantity} onChange={(e) => setItem(i, { quantity: e.target.value })} className="w-20 px-2 py-2 rounded-lg bg-white border border-slate-200 text-slate-900 text-sm" dir="ltr" />
              <input type="number" placeholder={ar ? 'سعر' : 'Price'} value={l.unitPrice} onChange={(e) => setItem(i, { unitPrice: e.target.value })} className="w-24 px-2 py-2 rounded-lg bg-white border border-slate-200 text-slate-900 text-sm" dir="ltr" />
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-slate-900 text-sm">{ar ? 'الإجمالي' : 'Total'}: {money(subtotal)}</span>
          </div>
        </div>
        <Field label={ar ? 'المبرر' : 'Justification'}><TextArea rows={2} value={form.justification} onChange={(e) => setForm({ ...form, justification: e.target.value })} /></Field>
      </Modal>

      {/* Convert to PO */}
      <Modal open={!!convertFor} onClose={() => setConvertFor(null)} title={ar ? 'تحويل لأمر شراء' : 'Convert to Purchase Order'}
        footer={<><button type="button" onClick={() => setConvertFor(null)} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm">{ar ? 'إلغاء' : 'Cancel'}</button>
          <PrimaryButton onClick={convert}>{ar ? 'إنشاء أمر شراء' : 'Create PO'}</PrimaryButton></>}>
        <Field label={ar ? 'المورّد' : 'Vendor'}><VendorSelect value={convVendor} onChange={(v) => setConvVendor(v)} placeholder={ar ? 'مورّد مقترح' : 'Suggested vendor'} /></Field>
        <p className="text-slate-500 text-xs">{ar ? `سيُطبّق ضريبة قيمة مضافة ${opts?.KSA_VAT_RATE ?? 15}%.` : `VAT ${opts?.KSA_VAT_RATE ?? 15}% will be applied.`}</p>
      </Modal>
    </div>
  );
}
