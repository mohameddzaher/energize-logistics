...
const ACTIVITY_TYPES = [
  { value: 'call', label: 'Call', icon: Phone, color: 'bg-blue-500/20 text-blue-400' },
  { value: 'email', label: 'Email', icon: Mail, color: 'bg-purple-500/20 text-purple-400' },
  { value: 'visit', label: 'Visit', icon: MapPin, color: 'bg-green-500/20 text-green-400' },
  { value: 'promise', label: 'Promise', icon: CheckCircle2, color: 'bg-yellow-500/20 text-yellow-400' },
  { value: 'follow_up', label: 'Follow-Up', icon: Clock, color: 'bg-orange-500/20 text-orange-400' },
  { value: 'whatsapp', label: 'WhatsApp', icon: MessageSquare, color: 'bg-emerald-500/20 text-emerald-400' },
  { value: 'note', label: 'Note', icon: FileText, color: 'bg-gray-500/20 text-gray-400' },
];
  ...
  const columns = [
    {
      key: 'createdAt',
      label: T.date,
      render: (val: string) => <span className="text-white text-xs">{formatDateTime(val)}</span>,
    },
    {
      key: 'type',
      label: T.type,
    ...
    },
    {
      ...
      render: (val: string) => val ? (
        <span className="text-gray-300 text-xs capitalize">{val}</span>
    ...
    },
    {
      ...
      label: T.notes,
      render: (val: string) => (
        <span className="text-gray-400 text-xs truncate max-w-[200px] block">{val || '-'}</span>
      ),
    },
    {
      key: 'amountCollected',
      ...
      render: (_: any, row: Activity) => {
        if (row.type === 'promise') {
          return row.promiseFulfilled ? (
            <span className="px-2 py-0.5 rounded text-xs bg-green-500/20 text-green-400">Fulfilled</span>
          ) : (
            <button
              type="button"
        ...
        if (row.type === 'follow_up' && row.followUpDate) {
          ...
          return (
            <span className={`px-2 py-0.5 rounded text-xs ${isOverdue ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{total} total activities</p>
        </div>
      ...
      <div className="flex gap-1 bg-gray-800 p-1 rounded-lg w-fit">
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            activeTab === 'activities' ? 'bg-[#f37121] text-white' : 'text-gray-400 hover:text-white'
          }`}
        >
          {T.title}
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${
            activeTab === 'followups' ? 'bg-[#f37121] text-white' : 'text-gray-400 hover:text-white'
          }`}
        >
          {T.followUps}
          {followUps.length > 0 && (
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-gray-400 text-xs mb-1.5">{T.activityType}</label>
                  <select
                    value={typeFilter}
        ...
        <div className="space-y-3">
          {followUps.length === 0 ? (
            ...
            followUps.map((fu) => {
              ...
              return (
                <div key={fu._id} className={`bg-gray-800 border rounded-xl p-4 ${isOverdue ? 'border-red-500/30' : 'border-gray-700'}`}>
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-white font-medium text-sm">{fu.customer?.companyName || 'Unknown'}</p>
                      <p className="text-gray-400 text-xs mt-1">{fu.notes || 'No notes'}</p>
                      {fu.invoice && (
                    ...
                    <div className="flex items-start gap-3">
                      <div className="text-right">
                        <p className={`text-xs font-medium ${isOverdue ? 'text-red-400' : 'text-orange-400'}`}>
                          {fu.followUpDate ? formatDate(fu.followUpDate) : '-'}
                        </p>
                        {isOverdue && (
                          <span className="text-xs text-red-400 flex items-center gap-1 mt-1 justify-end">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-500/10 text-green-400 text-xs font-medium border border-green-500/30 hover:bg-green-500/20 transition-colors"
                      >
            ...
            >
              <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between p-5 border-b border-gray-700">
                  <h2 className="text-white font-semibold text-lg">{T.markComplete}</h2>
                  <button type="button" onClick={() => setShowCompleteModal(false)} className="text-gray-400 hover:text-white">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="p-5 space-y-4">
                    ...
                    if (fu) {
                      return (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-3">
                          <p className="text-white text-sm font-medium">{fu.customer?.companyName || 'Unknown'}</p>
                  ...
                  <div>
                    <label className="block text-gray-400 text-xs mb-1.5">{T.completionNotes}</label>
                    <textarea
                      value={completionNotes}
                      onChange={(e) => setCompletionNotes(e.target.value)}
                      rows={3}
                      placeholder="Add notes about this follow-up completion..."
                      className="w-full px-3 py-2.5 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 resize-none"
                    />
                  </div>
            ...
            >
              <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between p-5 border-b border-gray-700 sticky top-0 bg-gray-800 z-10">
                  <h2 className="text-white font-semibold text-lg">{T.addActivity}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                  {modalError && (
                    <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400 text-sm">{modalError}</div>
                  )}
                  ...
                  <div>
                    <label className="block text-gray-400 text-xs mb-1.5">{T.invoice}</label>
                    {loadingInvoices ? (
                      <div className="w-full px-3 py-2.5 rounded-lg bg-gray-900 border border-gray-700 text-gray-500 text-sm">Loading...</div>
                    ) : (
                      <select
                        value={form.invoice}
                  ...
                  {/* Amount Collected */}
                  <div>
                    <label className="block text-gray-400 text-xs mb-1.5">{T.amountCollected}</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                  ...
                  {/* Next Follow-Up Date */}
                  <div>
                    <label className="block text-gray-400 text-xs mb-1.5">{T.followUpDate}</label>
                    <input
                      type="date"
                  ...
                  {form.type === 'promise' && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-gray-400 text-xs mb-1.5">{T.promiseDate} *</label>
                        <input
                          type="date"
                      ...
                      <div>
                        <label className="block text-gray-400 text-xs mb-1.5">{T.promiseAmount}</label>
                        <input
                          type="number"
                          step="0.01"
                  ...
                  <div>
                    <label className="block text-gray-400 text-xs mb-1.5">{T.notes}</label>
                    <textarea
                      value={form.notes}
