...
import { getRemoteTasksTranslations } from '@/lib/translations';
...
export default function RemoteTasksPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErr('');
    if (!assignTo || !title.trim()) { setErr(tx.errPickEmployeeAndTitle); return; }
    setSubmitting(true);
    try {
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><ListTodo className="w-6 h-6 text-[#f37121]" />{staff ? tx.teamTasks : tx.myTasks}</h1>
        <div className="flex items-center gap-2">
          ...
          {staff && (
            <button type="button" onClick={() => { setShowForm(true); setErr(''); }} className="flex items-center gap-2 px-4 py-2 bg-[#f37121] hover:bg-[#e0611a] text-white rounded-lg text-sm font-medium"><Plus className="w-4 h-4" />{tx.assignTask}</button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-slate-500 text-center py-10">{tx.noTasks}</p>
      ) : (
        <div className="space-y-2">
          {tasks.map((t) => (
            <div key={t._id} className={`bg-white border border-slate-200 rounded-xl p-4 flex items-start gap-3 ${t.done ? 'opacity-60' : ''} shadow-sm`}>
              <button type="button" onClick={() => toggle(t)} className={`mt-0.5 w-6 h-6 rounded-md border-2 flex items-center justify-center shrink-0 ${t.done ? 'bg-green-500 border-green-500' : 'border-slate-300 hover:border-[#f37121]'}`} title={tx.toggle}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-3 mt-1 text-xs text-slate-500">
                  {staff && t.user && <span>{tx.forLabel}: {t.user.firstName} {t.user.lastName}</span>}
                  {t.dueDate && <span>{tx.due}: {t.dueDate}</span>}
                  {t.assignedBy && <span>{tx.by}: {t.assignedBy.firstName} {t.assignedBy.lastName}</span>}
                </div>
              </div>
              {staff && (
                <button type="button" onClick={() => remove(t._id)} className="text-slate-500 hover:text-red-600 p-1" title={tx.delete}><Trash2 className="w-4 h-4" /></button>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-white border border-slate-200 rounded-xl w-full max-w-md shadow-2xl" dir={isRTL ? 'rtl' : 'ltr'}>
            <div className="flex items-center justify-between p-5 border-b border-slate-200">
              <h2 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold text-lg mb-3">{tx.assignTask}</h2>
            ...
            <form onSubmit={submit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-slate-700 text-sm mb-1.5">{tx.employee}</label>
                <select aria-label={tx.assignee} value={assignTo} onChange={(e) => setAssignTo(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm" required>
              ...
              <div>
                ...
                <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm" required placeholder={tx.titlePlaceholder} />
              </div>
              <div>
                <label className="block text-slate-700 text-sm mb-1.5">{tx.detailsOptional}</label>
              ...
              <div>
                <label className="block text-slate-700 text-sm mb-1.5">{tx.dueDateOptional}</label>
                <input aria-label={tx.dueDateOptional} type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm" />
              </div>
