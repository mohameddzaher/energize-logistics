  ...
  const columns = [
    {
      ...
      render: (_: any, row: Rep) => (
        <button type="button" onClick={(e) => { e.stopPropagation(); setSelectedRepId(row._id); }}
          ...
          <p className="text-white font-medium hover:text-[#f37121] transition-colors">{row.englishName}</p>
          {row.arabicName && <p className="text-gray-500 text-xs mt-0.5">{row.arabicName}</p>}
        </button>
      ),
    },
    { key: 'repId', label: T.repId, render: (v: string) => <span className="text-gray-400 text-xs font-mono">{v || '—'}</span> },
    {
    ...
    },
    {
      ...
      render: (_: any, row: Rep) => {
        if (!row.branch) return <span className="text-gray-500">—</span>;
        ...
        return <span className="text-gray-300 text-sm">{name}{showCity ? ` — ${city}` : ''}</span>;
      },
    },
    { key: 'monthlyTarget', label: T.monthlyTarget, render: (v: number) => <span className="text-white">{v}</span> },
    {
      key: 'isActive', label: T.repId === 'الحالة' ? 'الحالة' : 'Status',
      render: (_: any, row: Rep) => (
        <span className={`inline-flex items-center gap-1.5 text-xs ${row.isActive ? 'text-green-400' : 'text-gray-500'}`}>
          <span className={`w-2 h-2 rounded-full ${row.isActive ? 'bg-green-400' : 'bg-gray-500'}`} />
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={fetchReps} className="p-2 text-gray-400 hover:text-white rounded-lg hover:bg-gray-700" title="Refresh">
        ...
        <DataTable
          ...
          actions={(row: Rep) => (
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => openEdit(row)} className="p-1.5 text-gray-400 hover:text-white rounded hover:bg-gray-700" title={T.edit}>
                <Edit className="w-4 h-4" />
              </button>
              <button type="button" onClick={() => handleDelete(row)} className="p-1.5 text-gray-400 hover:text-red-400 rounded hover:bg-gray-700" title={T.delete}>
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-gray-300 text-sm font-medium mb-1.5">{T.arabicName}</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" value={form.repId} onChange={(e) => setForm({ ...form, repId: e.target.value })}
                        className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#f37121]/50 font-mono" />
                    </div>
                    <div>
                      <label className="block text-gray-300 text-sm font-medium mb-1.5">{T.phone}</label>
                  ...
                  <div>
                    <label className="block text-gray-300 text-sm font-medium mb-1.5">{T.joiningDate}</label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-gray-300 text-sm">{lang === 'ar' ? 'نشط' : 'Active'}</span>
                  </label>
