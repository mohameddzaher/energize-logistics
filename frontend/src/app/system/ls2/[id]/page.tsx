...
import {
  Truck, ArrowLeft, RefreshCw, Gauge, Thermometer, Fuel, Weight, Activity, Zap, MapPin, Wrench, Bell, Clock, Satellite, Route, AlertCircle, CheckCircle2, FileText, Fingerprint, Navigation, Droplet, MapPinned, FileDown, Loader2, User,
} from 'lucide-react';
...
export default function Ls2VehicleDetailPage() {
  ...
  // Mark-serviced — the shared dialog (same one the Maintenance page uses), so the
  // service checklist appears here too.
  ...
  return (
    <div className="space-y-5" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Vehicle identity (mirrored from Location Solutions). Always rendered:
          the top strip carries the two facts everyone opens this page for —
          the odometer RIGHT NOW (stamped with when it was read) and who is on
          the truck — even when the Wialon profile fields are empty. */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        ...
        <div className="flex flex-wrap items-stretch gap-3 mb-3">
          <div className="flex-1 min-w-[220px] bg-[#f37121]/5 border border-[#f37121]/20 rounded-lg px-4 py-3">
            <p className="text-xs font-semibold text-slate-700 flex items-center gap-1.5"><Gauge className="w-3.5 h-3.5 text-[#f37121]" /> {lang === 'ar' ? 'العداد الحالي' : 'Current odometer'}</p>
            <p className="text-2xl font-bold text-slate-900 tabular-nums leading-tight mt-1">{fmtKm(v.odometerKm)}</p>
            <p className="text-[11px] text-slate-600 mt-0.5">{lang === 'ar' ? 'وقت القراءة' : 'Read at'}: <b className="text-slate-800">{fmtDateTime(v.lastMessageAt, lang as Lang)}</b> ({timeAgo(v.lastMessageAt, lang as Lang)})</p>
          </div>
          <div className="flex-1 min-w-[220px] bg-slate-50 border border-slate-200 rounded-lg px-4 py-3">
            <p className="text-xs font-semibold text-slate-700 flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-[#f37121]" /> {t.driver}</p>
            <p className="text-xl font-bold text-slate-900 leading-tight mt-1 truncate" title={v.driver || ''}>{v.driver || '—'}</p>
            <p className="text-[11px] text-slate-600 mt-0.5">{lang === 'ar' ? 'السائق الحالي على المركبة' : 'Currently on this vehicle'}</p>
          </div>
        </div>
        {v.profile && (v.profile.vin || v.profile.brand || v.profile.simIccid) && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-2 text-sm pt-3 border-t border-slate-100">
        ...
        {cards.map((c) => {
          ...
          return (
            <div key={c.label} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              ...
              <p className="text-slate-600 text-xs font-medium mt-0.5">{c.label}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          ...
          <p className="text-xs text-slate-600 mb-3">{t.odometer}: <b className="text-slate-800 tabular-nums">{fmtKm(v.odometerKm)}</b></p>
              ...
              return (
                <div key={iv.id} className="border border-slate-100 rounded-lg p-3">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm font-medium text-slate-800 flex items-center gap-1.5 min-w-0"><Icon className={`w-4 h-4 shrink-0 ${iv.statusLevel === 'overdue' ? 'text-red-500' : iv.statusLevel === 'due' ? 'text-amber-500' : 'text-emerald-500'}`} /><span className="truncate">{iv.name}</span></p>
                    <span className={`shrink-0 px-2 py-0.5 rounded text-xs font-bold tabular-nums ${stI.bg} ${stI.text}`}>{isOver ? `−${fmtNum(Math.abs(r.value))}` : fmtNum(r.value)} {r.unit === 'km' ? (isOver ? t.kmOverdue : t.kmLeft) : r.unit}</span>
                  </div>
      ...
      {/* The complete maintenance record: deferrals, every service with its
      ...
      {svcModal && (
        <RegisterServiceModal
          unitId={v.unitId} interval={svcModal} currentOdo={v.odometerKm}
          lang={lang as Lang} isRTL={isRTL}
          onClose={() => setSvcModal(null)}
          onSaved={() => { setSvcModal(null); load(); }}
        />
      )}
    </div>
  );
...
function IdField({ label, value, mono }: { label: string; value?: string; mono?: boolean }) {
  ...
  return (
    <div className="min-w-0">
      ...
      <p className={`text-sm text-slate-900 truncate ${mono ? 'font-mono text-xs' : 'font-medium'}`} title={value}>{value}</p>
    </div>
  );
}
