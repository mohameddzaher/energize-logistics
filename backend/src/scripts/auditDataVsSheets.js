...
(async () => {
  ...
  // ── ملفّان من عندك يتناقضان ──────────────────────────────────────────────
  // ورقةُ «نوع الدفع للعملاء» تقول عن خمسةَ عشرَ عميلًا إنّهم ضريبيّون، ودفترُ
  // التحصيل يضعهم في ورقة الكاش ويعطيهم أكوادَ `C####` — وهي أكوادُ حسابات
  // الكاش. واثنان بالعكس. والاستيرادُ الأحدثُ (الدفتر) هو الذي استقرّ.
  //
  // ولا يُختار أحدُهما من هنا: نوعُ الدفع يقرّر إلى أيّ صفحةٍ يذهب كشفُ العميل،
  // فالخطأُ فيه يرسل كشفَ عميلٍ ضريبيّ إلى فواتير الكاش.
  if (ptBad === 0) ok('ونوعُ دفعه مطابق', true, `${wantPt.size} مطابق`);
  else needsYou('ورقةُ نوع الدفع تخالف دفترَ التحصيل',
    `${ptBad} عميلًا — أكثرُهم بكود C (حساب كاش في الدفتر) والورقةُ تقول ضريبي`);
