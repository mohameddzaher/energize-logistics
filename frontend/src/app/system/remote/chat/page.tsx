...
import { getRemoteChatTranslations } from '@/lib/translations';
  ...
  if (staff && !activeEmployee) {
    return (
      <div className="space-y-4" dir={isRTL ? 'rtl' : 'ltr'}>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><MessageSquare className="w-6 h-6 text-[#f37121]" />{tx.messages}</h1>
        {threads.length === 0 ? (
          <p className="text-slate-500 text-center py-10">{tx.noConversations}</p>
        ) : (
              ...
              <button key={t.employee._id} type="button" onClick={() => setActiveEmployee(t.employee._id)} className="w-full flex items-center justify-between gap-3 p-4 hover:bg-slate-100 text-start transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-slate-500 text-sm truncate">{t.lastMessage?.body || tx.noMessages}</p>
                  </div>
                </div>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center gap-3 pb-3 border-b border-slate-200">
        {staff && (
          <button type="button" onClick={() => { setActiveEmployee(null); setMessages([]); loadThreads(); }} className="text-slate-500 hover:text-slate-900 p-1" title={tx.back}><ArrowLeft className={`w-5 h-5 ${isRTL ? 'rotate-180' : ''}`} /></button>
        )}
        <MessageSquare className="w-5 h-5 text-[#f37121]" />
        <h1 className="text-lg font-bold text-slate-900">
          {staff && activeName ? `${activeName.employee.firstName} ${activeName.employee.lastName}` : tx.chatWithManager}
        </h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-3">
        {loading ? (
          ...
          <p className="text-slate-500 text-center py-10">{tx.startConversation}</p>
        ) : (
          messages.map((m) => {
      ...
      <form onSubmit={send} className="flex items-center gap-2 pt-3 border-t border-slate-200">
        ...
        />
        <button type="submit" disabled={sending || !text.trim()} className="w-12 h-12 rounded-full bg-[#f37121] hover:bg-[#e0611a] disabled:opacity-50 text-white flex items-center justify-center shrink-0" title={tx.send}>
