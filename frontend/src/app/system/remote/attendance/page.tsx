  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 py-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-white">{ar ? 'الحضور والانصراف' : 'Attendance'}</h1>
        <p className="text-gray-400 mt-1 text-lg">{new Date().toLocaleDateString(ar ? 'ar-EG' : 'en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
      </div>
      ...
      {checkedIn && (
        <div className="space-y-5">
          <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-5 text-center">
            <p className="text-green-400 text-lg font-medium">{ar ? 'أنت حاضر الآن' : 'You are checked in'}</p>
            <p className="text-white text-4xl font-bold mt-1">{fmtTime(record!.checkIn)}</p>
          </div>
          <BigButton color="red" onClick={toggle} busy={busy} icon={<LogOut className="w-14 h-14" />} label={ar ? 'تسجيل الانصراف' : 'Check Out'} />
        </div>
      )}
      {done && (
        <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8 text-center space-y-4">
          <CheckCircle2 className="w-16 h-16 text-green-400 mx-auto" />
          <p className="text-white text-2xl font-bold">{ar ? 'تم تسجيل يومك بالكامل' : 'Your day is recorded'}</p>
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Clock className="w-6 h-6 text-[#f37121]" />{ar ? 'سجل حضور الفريق' : 'Team Attendance'}</h1>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <input aria-label="From date" type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm" />
        </div>
        <div>
          <label className="block text-gray-400 text-xs mb-1">{ar ? 'إلى' : 'To'}</label>
          <input aria-label="To date" type="date" value={to} onChange={(e) => setTo(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm" />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-gray-400 text-center py-10">{ar ? 'لا توجد سجلات' : 'No records found'}</p>
      ) : (
        <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {records.map((r) => (
                <tr key={r._id} className="border-t border-gray-700/50">
                  <td className="px-4 py-3 text-white">{r.user ? `${r.user.firstName} ${r.user.lastName}` : '—'}</td>
                  <td className="px-4 py-3 text-gray-300">{r.date}</td>
                  <td className="px-4 py-3 text-gray-300">{fmtTime(r.checkIn)}</td>
                  <td className="px-4 py-3 text-gray-300">{r.checkOut ? fmtTime(r.checkOut) : '—'}</td>
                  <td className="px-4 py-3 text-gray-300">{fmtDuration(r.durationMinutes, ar ? 'ar' : 'en')}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs ${r.status === 'present' ? 'bg-green-500/20 text-green-400' : 'bg-amber-500/20 text-amber-400'}`}>
