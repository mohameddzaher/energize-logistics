/**
 * Seed the ls2 fleet-asset registry from data/masters/truck_tires_flat_6.json —
 * the workshop's sixth collection round (3 trucks: 5773, 7361, 7359), flat rows
 * identical in shape to round five (English section keys head/trailer/spare + a
 * bare hasSensor boolean). The six `spare` rows become الاستبن and — via the
 * import's section→isSpare rule — carry the first-class spare flag. Translated to
 * the exact /api/ls2/assets/import payload and pushed through the same idempotent
 * import (tires upsert by serial → a serial seen on another truck is a TRANSFER;
 * trailers by number; flatbeds by plate).
 *
 ...
 *
 * Run: node src/seeds/seedLs2AssetsFlat6.js
 */
require('dotenv').config({ path: require('path').join(__dirname, '../../.env') });
const mongoose = require('mongoose');
const rows = require('../data/masters/truck_tires_flat_6.json');
