...
import { Laptop, Plus, Edit, Undo2, Trash2, Check, Info } from 'lucide-react';
...
import {
  isItStaff, CustodyItem, EmployeeRef, CUSTODY_TYPES, CUSTODY_STATUSES, CONDITIONS,
  IT_CUSTODY_TYPE_KEYS, custodyTypeLabel, custodyStatusLabel, conditionLabel,
  optionsOf, empName, userName, fmtDate, fmtMoney, today, idOf,
} from '@/lib/it';
...
export default function ItCustodyPage() {
  ...
  const [returnForm, setReturnForm] = useState({ returnedCondition: 'good', returnedDate: '' });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (editing) await api.put(`/api/it/custody/${editing._id}`, form);
      else await api.post('/api/it/custody', form);
  ...
  const openReturn = (a: CustodyItem) => {
    setReturning(a);
    setReturnForm({ returnedCondition: a.condition || 'good', returnedDate: today() });
  };

  const doReturn = async () => {
    if (!returning) return;
    setSaving(true);
    try { await api.post(`/api/it/custody/${returning._id}/return`, returnForm); setReturning(null); load(); }
    catch (e: any) { alert(e.message); }
    setSaving(false);
  };
  ...
  const remove = async (a: CustodyItem) => {
    ...
    try { await api.delete(`/api/it/custody/${a._id}`); load(); } catch (e: any) { alert(e.message); }
  };
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard label={ar ? 'تم استرجاعها' : 'Returned'} value={returned} accent="text-green-600" />
        <StatCard label={ar ? 'إجمالي العهد' : 'Total items'} value={items.length} accent="text-slate-900" />
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="w-full sm:w-44 shrink-0">
          <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
            ...
            {optionsOf(CUSTODY_STATUSES).map((o) => <option key={o.key} value={o.key}>{ar ? o.ar : o.en}</option>)}
          </Select>
        </div>
      </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} wide
        ...
        footer={<>
          ...
          <PrimaryButton onClick={save} disabled={saving || !form.employee || !form.name.trim()}>
