...
// scored. Clicking a card opens that person's evaluation form.
...
export default function TeamBoard({ title, subtitle, department, section, showScopeToggle, children }: {
  ...
  const SCOPES: { key: Scope; ar: string; en: string }[] = [
    { key: 'managers', ar: 'مديرو الأقسام', en: 'Department managers' },
    { key: 'all', ar: 'كل الموظفين', en: 'All employees' },
  ];
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <PageHeader
        ...
        title={title || (ar ? 'تقييم الأداء' : 'Performance Evaluation')}
        subtitle={subtitle || `${periodLabel(period, lang as Lang)} · ${sum?.evaluated ?? 0}/${sum?.total ?? 0} ${ar ? 'تم تقييمهم' : 'evaluated'}`}
      >
          ...
          <div className="flex items-center bg-white border border-slate-200 rounded-lg p-0.5">
            {SCOPES.map((s) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${scope === s.key ? 'bg-[#f37121] text-white' : 'text-slate-600 hover:text-slate-900'}`}
              >
                {ar ? s.ar : s.en}
              </button>
            ))}
            ...
            return (
              ...
              >
                ...
                <div className="p-4 space-y-2">
                  ...
                  {/* Lock / edit-request state — a submitted evaluation is frozen
                      until the super admin approves an edit request. */}
                  {ev && (ev.locked || ev.editRequestStatus === 'pending' || ev.editRequestStatus === 'approved') && (
                    <div className="flex flex-wrap gap-1">
                      {ev.locked && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-slate-100 text-slate-600 border border-slate-200">
                          <Lock className="w-3 h-3" />
                          {ar ? 'مقفول — تم الإرسال' : 'Locked — submitted'}
                        </span>
                      )}
                      {ev.editRequestStatus === 'pending' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-100 text-amber-700 border border-amber-200">
                          {ar ? 'طلب تعديل قيد المراجعة' : 'Edit request under review'}
                        </span>
                      )}
                      {ev.editRequestStatus === 'approved' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-100 text-emerald-700 border border-emerald-200">
                          {ar ? 'التعديل مسموح' : 'Editing allowed'}
                        </span>
                      )}
                    </div>
                  )}
