...
export default function VehicleAnalyticsPage() {
  ...
  const tx = getVehicleAnalyticsTranslations(lang);
  ...
  const kpiCards = [
    { label: tx.totalFleet, value: kpis.totalFleet, icon: Truck, color: 'text-blue-600', bg: 'bg-blue-400/10' },
    { label: tx.activeVehicles, value: kpis.activeVehicles, icon: Activity, color: 'text-green-600', bg: 'bg-green-400/10' },
    { label: tx.totalRevenue, value: fmtNum(kpis.totalRevenue), icon: DollarSign, color: 'text-emerald-600', bg: 'bg-emerald-400/10' },
    { label: tx.totalGpsKm, value: fmtNum(kpis.totalGpsKm), icon: MapPin, color: 'text-purple-600', bg: 'bg-purple-400/10' },
    { label: tx.totalTrips, value: fmtNum(kpis.totalTrips), icon: Navigation, color: 'text-cyan-700', bg: 'bg-cyan-400/10' },
    { label: tx.fleetKms, value: fmtNum(kpis.fleetKms), icon: Route, color: 'text-indigo-600', bg: 'bg-indigo-400/10' },
    { label: tx.profitMargin, value: kpis.profitMargin.toFixed(1) + '%', icon: TrendingUp, color: kpis.profitMargin >= 0 ? 'text-green-600' : 'text-red-600', bg: kpis.profitMargin >= 0 ? 'bg-green-400/10' : 'bg-red-400/10' },
    { label: tx.alerts, value: kpis.alerts, icon: AlertTriangle, color: kpis.alerts > 0 ? 'text-amber-700' : 'text-slate-500', bg: kpis.alerts > 0 ? 'bg-amber-400/10' : 'bg-slate-100' },
  ];
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={() => setShowClearConfirm(false)}>
          <div className="bg-white border border-slate-200 rounded-xl p-6 max-w-md mx-4 shadow-sm" onClick={e => e.stopPropagation()}>
            <p className="text-slate-900 mb-4">{tx.clearConfirm}</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowClearConfirm(false)} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm hover:bg-slate-200">{tx.cancel}</button>
              <button onClick={handleClearAll} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700">{tx.confirm}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {!hasData ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {[{ title: `${tx.revenueByVehicle} (${tx.top15})`, data: revenueByVehicle, color: 'bg-emerald-500', textColor: 'text-emerald-600' },
              { title: `${tx.distanceRanking} (${tx.top15})`, data: distanceRanking, color: 'bg-purple-500', textColor: 'text-purple-600' }].map(chart => (
            ...
            {/* Fleet by Category */}
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3">{tx.fleetByCategory}</h3>
          ...
          {/* Vehicle Table */}
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3">{tx.title}</h3>
          ...
          {revenuePerKm.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3">{tx.revenuePerKm}</h3>
          ...
          {branchComparison.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3">{tx.branchComparison}</h3>
                ...
                {branchComparison.map(b => (
                  <div key={b.branch} className="bg-slate-100 rounded-lg p-3 border border-slate-300/60">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div><span className="text-slate-500">{tx.vehicles}:</span> <span className="text-blue-600 font-bold">{b.vehicles}</span></div>
                      <div><span className="text-slate-500">{tx.trips}:</span> <span className="text-cyan-700 font-bold">{b.trips}</span></div>
                      <div><span className="text-slate-500">{tx.totalRev}:</span> <span className="text-emerald-600 font-bold">{fmtNum(b.revenue)}</span></div>
                      <div><span className="text-slate-500">{tx.avgRevenue}:</span> <span className="text-amber-700 font-bold">{fmtNum(b.avgRev)}</span></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {vehicleUtilization.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3">{tx.vehicleUtilization}</h3>
          ...
          {topDetectedRoutes.length > 0 && (
            <div className="bg-white border border-indigo-500/40 rounded-xl p-4">
              <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3 flex items-center gap-2">
                <Route className="w-5 h-5 text-indigo-600" /> {tx.topRoutes}
              </h3>
              <div className="overflow-x-auto"><table className="w-full text-sm">
                <thead><tr className="bg-slate-900 text-slate-300 border-b border-slate-200">
          ...
          {filtered.htTrips.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <h3 className="bg-slate-900 px-3 py-2 rounded-lg text-white font-semibold mb-3">{tx.complianceOverview}</h3>
              <div className="space-y-3">
                {[{ label: tx.loaded, value: compliance.loaded }, { label: tx.unloaded, value: compliance.unloaded },
                  { label: tx.photoSent, value: compliance.photoSent }, { label: tx.receiptDelivered, value: compliance.receiptDelivered }].map(item => {
