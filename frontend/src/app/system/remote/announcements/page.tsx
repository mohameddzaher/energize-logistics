  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Megaphone className="w-6 h-6 text-[#f37121]" />{ar ? 'الإعلانات' : 'Announcements'}</h1>
      ...
      {loading ? (
        ...
        <p className="text-gray-400 text-center py-10">{ar ? 'لا توجد إعلانات' : 'No announcements'}</p>
      ) : (
        <div className="space-y-3">
          {items.map((a) => (
            <div key={a._id} className="bg-gray-800 border border-gray-700 rounded-xl p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="text-white font-semibold">{a.title}</h3>
                  <p className="text-gray-300 text-sm mt-1 whitespace-pre-wrap">{a.body}</p>
                  <p className="text-gray-500 text-xs mt-2">
                    {a.author ? `${a.author.firstName} ${a.author.lastName} · ` : ''}{fmtDateTime(a.createdAt)}
                  </p>
                </div>
                {staff && (
                  <button type="button" onClick={() => remove(a._id)} className="text-gray-500 hover:text-red-400 p-1 shrink-0" title="Delete"><Trash2 className="w-4 h-4" /></button>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-gray-800 border border-gray-700 rounded-xl w-full max-w-md shadow-2xl" dir={isRTL ? 'rtl' : 'ltr'}>
            <div className="flex items-center justify-between p-5 border-b border-gray-700">
              <h2 className="text-white font-semibold text-lg">{ar ? 'إعلان جديد' : 'New Announcement'}</h2>
            ...
            <form onSubmit={submit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-gray-300 text-sm mb-1.5">{ar ? 'العنوان' : 'Title'}</label>
                <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-3 py-2.5 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm" required />
              </div>
              <div>
                <label className="block text-gray-300 text-sm mb-1.5">{ar ? 'النص' : 'Body'}</label>
                <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={4} className="w-full px-3 py-2.5 rounded-lg bg-gray-900 border border-gray-700 text-white text-sm" required />
              </div>
