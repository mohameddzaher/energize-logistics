    ...
    <div className="max-w-5xl mx-auto" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {!parseResult && !parsing && (
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors ${
            dragOver ? 'border-[#f37121] bg-[#f37121]/10' : 'border-slate-300 hover:border-[#f37121]/70 bg-slate-50'
          }`}
        >
          <Upload className="w-12 h-12 text-slate-500 mx-auto mb-3" />
          <p className="text-slate-900 text-lg font-semibold">{T.dropHere}</p>
      ...
      {parsing && (
        <div className="bg-white rounded-xl p-8 text-center border border-slate-200 shadow-sm">
          ...
          <p className="text-slate-700">{T.parsing}</p>
        </div>
      )}
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 my-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-red-700 font-semibold text-sm">{T.errorTitle}</p>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 mt-4 shadow-sm">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              <FileSpreadsheet className="w-6 h-6" style={{ color: ORANGE }} />
              <div>
                <p className="text-slate-900 font-semibold text-sm">{file?.name}</p>
                <p className="text-slate-500 text-xs mt-0.5">
                  {T.rowsDetected}: <span className="text-slate-900 font-bold">{parseResult.totalRows}</span>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <button
                    ...
                    className="text-yellow-700 hover:text-yellow-100 text-xs mt-2 inline-flex items-center gap-1"
                  >
                  ...
                  {showRowDetails && (
                    <div className="mt-3 max-h-56 overflow-y-auto border border-yellow-500/20 rounded p-2 bg-black/20">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-slate-900 text-yellow-200/80 text-start">
          ...
          <div className="mt-5 flex items-center gap-3 flex-wrap">
            ...
            {generating && (
              <button
                ...
                className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 text-sm flex items-center gap-2"
              >
                <X className="w-4 h-4" />
                {T.cancel}
              </button>
            ...
            {zipBlob && (
              <>
                ...
                <button
                  onClick={reset}
                  className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-sm flex items-center gap-2"
                >
          ...
          {generating && progress && (
            <div className="mt-5">
              <div className="flex items-center justify-between text-sm mb-2">
                ...
                <span className="text-slate-900 font-mono">{progressPct}%</span>
              </div>
              ...
              <p className="text-slate-500 text-xs mt-2 truncate" title={progress.currentFileName}>
